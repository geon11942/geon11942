...
public class Inventory : MonoBehaviour
{
            ...
            for (int i = 0; i < slots.Length; i++)
            {
                if (slots[i].CheckNeedItem())
                {
                        ...
                        SlotPos.y - 0.35f < MousePos.y && SlotPos.y + 0.35f > MousePos.y)
                    {
                        ...
                        {
                            ItemDataTable Idata = slots[i].ItemGS;
    ...
    public bool UnEquipItem(ItemDataTable item)
    {
        for(int i = 0; i < slots.Length; i++)
        {
            if (!slots[i].CheckNeedItem())
    ...
    public GameObject ItemGS
    {
        ...
        set 
        {
            ...
            for (int i = 0; i < slots.Length; i++)
            {
                if (slots[i].CheckNeedItem())
                {
                    if (slots[i].ItemGS.Item_ID == item.GetComponent<ItemData>().ItemDataTable.Item_ID && slots[i].ItemGS.MaxStack > 1 && slots[i].ItemGS.Stack < slots[i].ItemGS.MaxStack)
                    {
                        if (slots[i].ItemGS.Stack + item.GetComponent<ItemData>().ItemDataTable.Stack > slots[i].ItemGS.MaxStack)
                        {
                            item.GetComponent<ItemData>().ItemDataTable.Stack = -(slots[i].ItemGS.MaxStack - slots[i].ItemGS.Stack);
                            slots[i].ItemGS.Stack = slots[i].ItemGS.MaxStack - slots[i].ItemGS.Stack;
                            slots[i].CheckStack();
                            Debug.Log("아이템 스택 초과.");
                        }
                        else
                        {
                            slots[i].ItemGS.Stack = item.GetComponent<ItemData>().ItemDataTable.Stack;
                            slots[i].CheckStack();
                            InvenFull = false;
                            item = null;
                            break;
            ...
            if (item != null)
            {
                for (int i = 0; i < slots.Length; i++)
                {
                    if (!slots[i].CheckNeedItem())
                    {
                        slots[i].ItemGS = item.GetComponent<ItemData>().ItemDataTable;
